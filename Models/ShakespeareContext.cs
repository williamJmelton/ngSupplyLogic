...
#nullable disable
    ...
    // public decimal CheckPrice(int barCode)
    // {
    //   var ObjID = GetItemObjIDFromScancode(barCode);
    //   var _itemResult =
    //       from item in Item
    //       where item.ObjID == ObjID
    //       select new { item.ListPrice };
...
#nullable disable
    ...
    // This function is not complete is and part of the feature
    // to show the invoice paid status and payment method 

    // public void CheckInvoicePaidStatus(string InvoiceOID)
    // {
    //     var results = new List<dynamic>();

    //     var _dbResults =
    //         from _ArTrans in Artransaction
    //         where _ArTrans.SalesOrderOID == InvoiceOID
    //         select new {  }

    // }

    public int GetSalesOrdersForCurrentMonth(int customerNum)
    {
      Console.WriteLine("Getting Marl count for customer #" + customerNum);
      List<int> SalesOrderOIDs = new List<int>();
      List<int> LineResults = new List<int>();
      int? CustomerOID = 0;

      //Console.WriteLine("Trying to find the marlboro...");

      // let's first convert that customer Number into the sql-friendly OID....
      int? _custRes =
      (from _cust in Customer
       where _cust.Num == customerNum
       select _cust.ObjId).FirstOrDefault();

      if (_custRes.HasValue)
      {
         CustomerOID = _custRes;
      } else
        {
            //Console.WriteLine("Couldn't find the customer number. PANIC");
        }

      // Here we get all the invoices that the customer has made in the past month
      var _results =
      from _SalesOrders in SalesOrder
      where _SalesOrders.CustomerOid == CustomerOID &&
      _SalesOrders.InvoicedDate.Value.Month == DateTime.Now.Month &&
      _SalesOrders.InvoicedDate.Value.Year == DateTime.Now.Year
      orderby _SalesOrders.InvoicedDate descending
      select _SalesOrders.ObjId;

      //Console.WriteLine("Got invoices for customer. Storing in variable.");

      SalesOrderOIDs = _results.ToList<int>(); // and here we store all those Invoice OID's into a variable

      //Console.WriteLine("Stored invoices in a variable.");

            //Console.WriteLine("We have " + SalesOrderOIDs.Count + " Invoices to look at.");
      // Here, we gonna have to loop through each invoice's individual
      // lines so that we can see if any line was marlboro, and add them all up.
      foreach (int OID in SalesOrderOIDs)
      {
        //Console.WriteLine("Searching Sales Order OID " + OID + " for Marlboro.");

                List<int?> _lineResults =
                (from _line in SalesOrderLine
                 where _line.SalesOrderOid == OID && _line.ClassOid == 274
                 select _line.OrderedQty).ToList();

        foreach(int? qty in _lineResults)
                {
                    int? _lineResult = qty.HasValue ? qty : 0;

                    //Console.WriteLine("Found " + _lineResult.Value + "Marlboro");

                    if (qty.HasValue)
                    {
                        LineResults.Add(qty.HasValue ? _lineResult.Value : 0);
                        //Console.WriteLine("Line had " + _lineResult.Value + " Marlboros on it.");
                    }
                }
      }
      Debug.WriteLine("Marlboro Amount is :" + LineResults.Sum() + " for customer #" + customerNum);
      return LineResults.Sum();
    }

    public int GetSalesOrdersForSelectedMonth(int customerNum, int monthNum)
    {
      Console.WriteLine("Getting Marl count for customer #" + customerNum);
      List<int> SalesOrderOIDs = new List<int>();
      List<int> LineResults = new List<int>();
      ...
      from _SalesOrders in SalesOrder
      where _SalesOrders.CustomerOid == CustomerOID &&
      _SalesOrders.InvoicedDate.Value.Month == monthNum &&
      _SalesOrders.InvoicedDate.Value.Year == DateTime.Now.Year
      orderby _SalesOrders.InvoicedDate descending
      select _SalesOrders.ObjId;
